...
const CoinBookForm = () => {
  ...
  const displayList = section === '미체결' ? holding : completed;
  ...
  return (
    <StyledBookContainer>
      <StyledBookTitle>
        <button
          onClick={() => setSection('미체결')}
          className={section === '미체결' ? 'active-pending' : ''}
        >미체결</button>
        <button
          onClick={() => setSection('체결')}
            ...
            >
              <StyledContent className="!ml-0">
                ...
                <StyledAmount className="mt-1">
                  <p className="text-zinc-400 font-mono">{order.orderQuantity.toFixed(3)} <span className="text-[10px]">{coinUnit}</span></p>
